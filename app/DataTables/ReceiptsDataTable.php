...
class ReceiptsDataTable extends DataTable
{
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('partyable_code', function ($model) {
                ...
                $date=date_create($model->payment_date);
                return date_format($date,"d-m-Y");
            })->editColumn('amount',function($model){
                return  str_replace(',','.',number_format($model->amount));
    ...
    {
        $query = $model->with('transaction_type:id,name')->where('payment_method_id',1);
    ...
    public function html()
    {
        ...
        if(auth()->user()->hasRole('admin|QS')){
            $buttons =  [
                [
                    'text' => "<i class='feather icon-plus'></i> Thêm phiếu thu",
